...
static void update_screen(void)
{
	...
	if (varblock.min_i < 9999) {
		...
		fprintf(stderr, "Dirty page: %dx%d+%d+%d...\n",
		  (varblock.max_i + 2) - varblock.min_i,
		  (varblock.max_j + 1) - varblock.min_j,
...
int input_finder(int max_num, const char* *patterns, char *path, int path_size)
{
	...
	for (i = 0; i < max_num; i++)
	{
		...
		for (keystr = patterns; *keystr; keystr++) {
			if (strstr(name, *keystr)) {
				printf("Found input device %s by keyword %s\n",
							path, *keystr);
...
int input_search()
{
	...
	if (input_finder(max_input_num, KBD_PATTERNS,
		...
		printf("Cannot automatically find the keyboard device\n");
		rc++;
	}

	if (input_finder(max_input_num, TOUCH_PATTERNS,
			TOUCH_DEVICE, sizeof TOUCH_DEVICE) < 0) {
		printf("Cannot automatically find the touchscreen device\n");
		rc++;
	}
	return rc;
}
...
int main(int argc, char **argv)
{
	...
	printf("Initializing Framebuffer VNC server:\n");
	printf("	width:  %d\n", (int)scrinfo.xres);
	printf("	height: %d\n", (int)scrinfo.yres);
	printf("	bpp:    %d\n", (int)scrinfo.bits_per_pixel);
	printf("	port:   %d\n", (int)VNC_PORT);
